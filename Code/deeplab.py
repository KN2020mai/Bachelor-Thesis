...
class ResNet(nn.Module):
    def __init__(self, in_channels, block, layers, output_stride, pretrained=True):
        self.inplanes = 64
    ...
    def _load_pretrained_model(self):
        ...
        for k, v in pretrain_dict.items():
            if k in state_dict:
                if (not self.in_channels == 3) and k == "conv1.weight":
...
def ResNet101(in_channels=3, output_stride=8, pretrained=True):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(in_channels, Bottleneck, [3, 4, 23, 3], output_stride, pretrained=pretrained)
    return model

def build_backbone(backbone, in_channels, output_stride):
    if backbone == 'resnet':
        return ResNet101(in_channels, output_stride)
...
class DeepLab(nn.Module):
    def __init__(self, backbone='resnet', in_channels=1, output_stride=16, num_classes=21):
