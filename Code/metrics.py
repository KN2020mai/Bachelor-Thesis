...
def semantic_segmentation_metrics(label_true, label_pred):
    label_true = label_true > 0
    label_pred = label_pred > 0
    hist = np.bincount(2 * label_true.astype(int).flatten() + label_pred.astype(int).flatten(), minlength=2 ** 2).reshape(2, 2)
    ...
    return acc, precision, recall, mean_iu

def acc_prec_rec_iou(label_trues, label_preds):
    acc_prec_rec_ious = []
    for label_true, label_pred in zip(label_trues, label_preds):
        acc, precision, recall, mean_iu = semantic_segmentation_metrics(label_true, label_pred)
        acc_prec_rec_ious.append([acc, precision, recall, mean_iu])
    return np.array(acc_prec_rec_ious)
